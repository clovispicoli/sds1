...
const CreateRecord = () => {
  ...
  const [selectdGame, setSelectdGame] = useState('');
  ...
  const handleChangePlatform = (selectdPlatform: GamePlatform) => {
    ...
    const ganesByPlatform = allGames.filter(
      game => game.platform === selectdPlatform
    )
    setFilteredGames(ganesByPlatform);
  }

  const handleSubmit = () => {
    const payload = { name, age, gameId: selectdGame};
